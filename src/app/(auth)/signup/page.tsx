    ...
    return (
      <div className="min-h-screen bg-transparent flex items-center justify-center p-4">
        <div className="text-center max-w-sm bg-[var(--kb-surface)]/90 backdrop-blur-md border border-[var(--kb-border)] rounded-lg p-8">
          ...
          <Link href="/login" className="inline-block mt-6 text-xs text-[#FFDE4D] hover:underline">
            ← Back to sign in
          </Link>
        </div>
      </div>
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className={[
                'w-full bg-zinc-900 border text-white text-sm px-3 py-2.5 rounded-sm focus:outline-none transition-colors',
                confirmPassword && password !== confirmPassword
                  ? 'border-red-700 focus:border-red-500'
                  : 'border-zinc-800 focus:border-[#FFDE4D]',
              ].join(' ')}
